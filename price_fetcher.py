...
def get_prices_from_skinport(item_names: list[str]) -> dict[str, float]:
    ...
    try:
        ...
        response = requests.get(SKINPORT_API_URL, params=params, headers=headers)
        response.raise_for_status()

        all_items = response.json()

        # Create a lookup table for faster access
        skinport_prices = {item['market_hash_name']: item.get('suggested_price') for item in all_items}
