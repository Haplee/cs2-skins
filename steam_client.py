...
def get_inventory(steam_id: str, use_test_data: bool = False) -> list[str]:
    ...
    try:
        ...
        if data.get('success') == 1 and 'descriptions' in data:
            ...
            descriptions = {f"{desc['classid']}_{desc.get('instanceid', '0')}": desc['market_hash_name'] for desc in data['descriptions']}
            ...
            print(f"Failed to fetch inventory. Response: {data.get('error', 'No descriptions found')}")
            return []
        ...
        print("Failed to decode JSON from response. The user's inventory might be private.")
        return []
...
if __name__ == '__main__':
    ...
    example_steam_id = "76561197960435530" # A known public inventory for testing
